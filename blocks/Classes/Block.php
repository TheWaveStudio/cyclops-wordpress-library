...
class Block
{
   ...
   * @return $this
   */
  public function addBlockControls(string $name, array $fields = [])
  {
    if (!is_array($fields)) return $this;

    $this->blockControls[] = ['title' => $name, 'fields' => $fields];
    return $this;
  }
  ...
  public function register($hook = '')
  {
    ...
    $blocksControl = map($this->blockControls ?? [], function ($control) {
      return [
        'title' => $control['title'] ?? __('Options', 'cy'),
        'fields' => map($control['fields'] ?? [], fn($field) => $field->getField())
      ];
    });
